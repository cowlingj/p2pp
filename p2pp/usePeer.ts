...
export default function usePeer(options: Options) {
    ...
    const [peer, setPeer] = useState<Peer | undefined>(() => new Peer(uniqueNamesGenerator({ dictionaries: [adjectives, colors, animals], separator: "-" }), { host: options.host, port: options.port, secure: options.secure }));
    ...
    useEffect(() => {
        peer?.destroy();
        const newPeer = new Peer(uniqueNamesGenerator({ dictionaries: [adjectives, colors, animals], separator: "-" }), { host: options.host, port: options.port, secure: options.secure });
        setPeer(newPeer);
        setConnectionInfo({ id: newPeer.id, connections: [], errors: [] });
    }, [options.host, options.port, options.secure])
    ...
    useEffect(() => {
        ...
        peer.on('error', (e) => {
            console.log('err', connectionInfoRef.current)
